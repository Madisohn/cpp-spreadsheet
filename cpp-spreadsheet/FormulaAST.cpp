...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
    double Evaluate(const SheetInterface& sheet) const override {
        ...
        switch (type_) {
        case Add :
            if(lhs_.get()->Evaluate(sheet) + rhs_.get()->Evaluate(sheet) == inf) throw FormulaError(FormulaError::Category::Div0);
        ...
        case Multiply :
            ...
            return lhs_.get()->Evaluate(sheet) * rhs_.get()->Evaluate(sheet);
        case Divide :
            if (rhs_.get()->Evaluate(sheet) == 0 || lhs_.get()->Evaluate(sheet) / rhs_.get()->Evaluate(sheet) == inf) throw FormulaError(FormulaError::Category::Div0);
            return lhs_.get()->Evaluate(sheet) / rhs_.get()->Evaluate(sheet);
        default :
            return 0;
        }
...
// Реализуйте метод Evaluate() для унарных операций.
    double Evaluate(const SheetInterface& sheet) const override {
        switch (type_) {
        case UnaryMinus :
            return operand_.get()->Evaluate(sheet) *  -1;
        case UnaryPlus :
...
public:
    ...
    double Evaluate(const SheetInterface& sheet) const override {
        if (sheet.GetCell(pos_) == nullptr) return 0;
        else if (std::holds_alternative<double>(sheet.GetCell(pos_)->GetValue()))
            return std::get<double>(sheet.GetCell(pos_)->GetValue());
        else throw FormulaError(FormulaError::Category::Value); // ВСТАВИТЬ НУЖНУЮ ОШИБКУ!!
    }
private:
    Position pos_;
};
...
public:
    ...
    void exitCell(FormulaParser::CellContext* ctx) override {
        ...
        auto node = std::make_unique<CellExpr>(pos);
        args_.push_back(std::move(node));
    }
...
FormulaAST ParseFormulaAST(std::istream& in) {
    ...
    // Поиск ячеек участвующих в формуле
    std::forward_list<Position> referenced_cells;
    for (auto token : tokens.getTokens()) {
        if (token->getType() == 8) referenced_cells.push_front(Position::FromString(token->getText()));
    }

    FormulaAST formula = FormulaAST(listener.MoveRoot());
    formula.SetReferencedCells(referenced_cells);
    return formula;
}
...
void FormulaAST::SetReferencedCells(std::forward_list<Position> cells) {
    cells_ = cells;
}

std::forward_list<Position> FormulaAST::GetReferencedCells() const {
    return cells_;
}
