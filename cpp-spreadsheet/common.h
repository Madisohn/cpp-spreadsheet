...
public:
	...
	virtual void Set(SheetInterface& sheet, Position pos, std::string text) = 0;
...
public:
	...
	virtual std::map<Position, std::set<Position>>& GetReferencedCellsMap() = 0;
	virtual std::map<Position, std::vector<std::pair<Position, int>>>& GetGraph() = 0;
};
